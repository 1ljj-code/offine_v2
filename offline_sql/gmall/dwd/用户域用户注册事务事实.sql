...
CREATE
EXTERNAL TABLE bigdata_offline_v1_ws.dwd_user_register_inc
(
    `user_id`        STRING,
    `date_id`        STRING,
    `create_time`    STRING,
...
insert
overwrite table bigdata_offline_v1_ws.dwd_user_register_inc partition(ds = '20250916')
select ui.user_id,
       ...
       model,
       operate_system
from (select id user_id, -- 直接使用独立字段id，替代data.id
      ...
      where dt = ${bizdate} -- 无type字段，按分区筛选首日全量注册数据
         ...
         left join
     (select get_json_object(log, '$.common.uid') as user_id,
...
CREATE
EXTERNAL TABLE bigdata_offline_v1_ws.dwd_user_login_inc
(
    `user_id`        STRING,
    `date_id`        STRING,
    `login_time`     STRING,
...
insert
overwrite table bigdata_offline_v1_ws.dwd_user_login_inc partition (ds = '20250916')
select get_json_object(log, '$.common.uid') as user_id,
       -- 解析ts转换为日期ID
       date_format(from_utc_timestamp(cast(get_json_object(log, '$.ts') as bigint) / 1000, 'GMT+8'),
                   'yyyy-MM-dd')               date_id,
       -- 解析ts转换为登录时间
       date_format(from_utc_timestamp(cast(get_json_object(log, '$.ts') as bigint) / 1000, 'GMT+8'),
                   'yyyy-MM-dd HH:mm:ss')      login_time,
       -- 解析log字段中的渠道、设备等信息
...
from (select log,
      ...
      where dt = ${bizdate}
        ...
        and get_json_object(log, '$.common.uid') is not null) t
where t.rn = 1;